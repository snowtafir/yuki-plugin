...
class Config {
     ...
     */
    getGuobaConfigFilePath(appDir, functionName) {
        return path__default.join(_paths.botYukiData, 'config', appDir, `${functionName}.guoba.support.yaml`);
    }
    /**
     * 检查 Guoba 配置文件是否存在
     * @param appDir 配置app目录
     ...
     */
    hasGuobaConfig(appDir, functionName) {
     ...
     */
    readGuobaConfig(appDir, functionName) {
        try {
            const guobaPath = this.getGuobaConfigFilePath(appDir, functionName);
