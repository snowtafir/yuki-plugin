...
import { Plugin, Redis, Segment } from 'yunzaijs';
...
import { readLoginCookie, applyLoginQRCode, pollLoginQRCode, saveLoginCookie, postGateway, exitBiliLogin, checkBiliLogin, readSavedCookieItems, saveLocalBiliCk, readSyncCookie, getNewTempCk, saveTempCk } from '../models/bilibili/bilibili.mian.models.js';
    ...
    constructor() {
        ...
        this.rule = [
            ...
            },
            {
                ...
                fnc: this.getVideoInfoByAid_BV.name
            }
        ];
    ...
    async reflashTempCk() {
        try {
            ...
            if (newTempCk) {
                ...
                if (data?.code !== 0) {
                    global?.logger?.error(`优纪插件：tempCK，Gateway校验失败：${JSON.stringify(data)}`);
                }
                else if (data?.code === 0) {
                    global?.logger?.mark(`优纪插件：tempCK，Gateway校验成功：${JSON.stringify(data)}`);
                }
