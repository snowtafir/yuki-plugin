...
export class WeiboTask {
  ...
  async runTask() {
    ...
    let dynamicTimeRange: number = weiboConfigData.dynamicTimeRange || 7200; // 筛选何时发布的动态，单位为秒，默认2小时内发布的动态
  ...
  async makeDynamicMessageMap(
    ...
    if (!!weiboConfigData.pushMsgMode) {
      ...
      let isPauseGif: boolean = !!weiboConfigData?.isPauseGif === true ? true : false; // 是否暂停 GIF 动图，默认为 false
      ...
      for (const [bot_id, chatMap] of botMap) {
        for (const [chatId, messageCombinationList] of chatMap) {
          ...
          if (forwardSendDynamic && useForward) {
            ...
            // 尝试合并转发动态
            if (
              (await this.sendMsgApi(chatId, bot_id, chatType, '优纪酱微博动态通知~')) &&
